    ...
    for (const war of (war_log?.items || [])) {
        ...
        for (const member_in_war of war.clan.members) {
            if (player_map.has(member_in_war.tag) && member_in_war.attacks) {
                ...
                // Use the TH level from the war log if available, otherwise fallback to current
...
const calculateArchiveStats = (war_list) => {
    ...
    const totalStars = war_list.reduce((sum, w) => sum + (w.clan?.stars || 0), 0);
    const totalDestruction = war_list.reduce((sum, w) => sum + (w.clan?.destructionPercentage || 0), 0);
    return {
...
app.get('/api/player-roster', async (req, res) => {
    ...
    try {
        const [clanRes, warLogRes] = await Promise.all([ cocApi.get(`/clans/${encodedTag}`), cocApi.get(`/clans/${encodedTag}/warlog`) ]);
...
app.get('/api/war-log-stats', async (req, res) => {
    ...
    res.json({
        data: {
            cwl: { wars: cwl_wars, stats: cwl_stats },
            regular: { wars: regular_wars, stats: regular_stats }
        },
        error: null
    });
});
