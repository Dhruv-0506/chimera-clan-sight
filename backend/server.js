...
app.get('/api/war-log-stats', async (req, res) => {
    ...
    const war_list = (data.items || []).filter(war => war.clan.attacks !== undefined);
    const stats = calculateArchiveStats(war_list);
    res.json({ data: { wars: war_list, stats: stats }, error: null });
});
