...
const formatDate = (endTimeString?: string | null) => {
...
const processWarList = (wars: any[]) =>
  wars.map((war, idx) => ({
    ...
    result: (war.result ?? 'Draw').toLowerCase().replace(/^(.)/, (_, c) => c.toUpperCase()),
...
export default function Archives() {
  ...
  useEffect(() => {
    const fetchArchiveData = async () => {
      try {
        ...
        const wins = rawWars.filter((w: any) => w.result === 'win').length;
        const totalStars = rawWars.reduce((s: number, w: any) => s + (w.clan?.stars ?? 0), 0);
        const totalDest = rawWars.reduce((d: number, w: any) => d + (w.clan?.destructionPercentage ?? 0), 0);
  ...
  if (isLoading) return <div>Loading War Archives...</div>;
  if (error || !stats) return <div style={{ color: 'red' }}>{error ?? 'No war data'}</div>;
